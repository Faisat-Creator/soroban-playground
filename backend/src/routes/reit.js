...
router.post(
  ...
  rateLimitMiddleware('invoke'),
  asyncHandler(async (req, res, next) => {
    const { contractId, admin, name, totalShares, pricePerShare, network } =
      req.body || {};
...
router.post(
  ...
  rateLimitMiddleware('invoke'),
  asyncHandler(async (req, res, next) => {
    const { contractId, investor, propertyId, shares, network } =
      req.body || {};
    const errs = requireFields(req.body, [
      'contractId',
      'investor',
    ...
    if (typeof propertyId !== 'number' || propertyId < 1)
      return next(createHttpError(400, 'propertyId must be >= 1'));
    if (typeof shares !== 'number' || shares < 1)
      return next(createHttpError(400, 'shares must be >= 1'));
    const result = await invoke(
      contractId,
      'mint_shares',
      { investor, property_id: propertyId, shares },
      network
    );
...
router.post(
  ...
  rateLimitMiddleware('invoke'),
  asyncHandler(async (req, res, next) => {
    const { contractId, from, to, propertyId, shares, network } =
      req.body || {};
    const errs = requireFields(req.body, [
      'contractId',
      'from',
    ...
    const result = await invoke(
      contractId,
      'transfer_shares',
      { from, to, property_id: propertyId, shares },
      network
    );
    return res.json({ success: true, output: result.parsed });
  })
...
router.post(
  ...
  asyncHandler(async (req, res, next) => {
    ...
    const result = await invoke(
      contractId,
      'deposit_dividends',
      { admin, property_id: propertyId, amount },
      network
    );
    return res.json({ success: true, output: result.parsed });
  })
...
router.post(
  ...
  asyncHandler(async (req, res, next) => {
    ...
    const result = await invoke(
      contractId,
      'claim_dividends',
      { investor, property_id: propertyId },
      network
    );
