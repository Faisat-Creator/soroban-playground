...
export class TreasuryService {
  ...
  async getProposals() {
    return await this.db.all('SELECT * FROM treasury_proposals ORDER BY created_at DESC');
  }

  async getProposalById(id) {
    const proposal = await this.db.get('SELECT * FROM treasury_proposals WHERE contract_tx_id = ?', [id]);
    if (proposal) {
      proposal.approvals = await this.db.all('SELECT signer, timestamp FROM treasury_approvals WHERE proposal_id = ?', [id]);
    }
    return proposal;
  }
  ...
  async getHistory() {
    return await this.db.all('SELECT * FROM treasury_history ORDER BY timestamp DESC LIMIT 100');
  }
